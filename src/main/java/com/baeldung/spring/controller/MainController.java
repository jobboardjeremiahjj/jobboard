...
public class MainController {
	...
	public String showHomePage() {
            System.out.println("--------------------------------------u in main contr index");
		return "index";
	}
	...
	public String login(@RequestParam("emailId") String emailId, @RequestParam("password") String password,
		...
		if (type.equals("recruiter")) {
			list = companyDao.PasswordLookUp(email);
				System.out.println("------ is a recruiter");
			if (list.size() == 0) {
...
//                                        System.out.println("company id is " + cidl);
					Company cmp = companyDao.getCompanyFromEmail(email);
                                        System.out.println("company is " + cmp.getCompanyName());
			...
			if (list.size() == 0) {
				...
				if (pwd.equals(list.get(0))) {
					...
					JobSeeker js = jobSeekerDao.getJobSeekerFromEmail(email);
