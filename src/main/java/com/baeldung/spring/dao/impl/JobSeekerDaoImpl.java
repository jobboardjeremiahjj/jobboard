...
public class JobSeekerDaoImpl implements JobSeekerDao {
	...
	public List<?> filterJobs(JobPostingsView jpv, List<?> jobIds) {
		...
		String selectQuery = "SELECT jpv FROM JobPosting jpv";
...
//		}
	...
	public JobSeeker updateJobSeeker(JobSeeker js) {
		...
		jobseeker.setWorkEx(js.getWorkEx());
                jobseeker.setAge(js.getAge());
                jobseeker.setEdClass(js.getEdClass());
		try {
			if (jobseeker != null) {
...
//			list.add(uid);
        ...
        @Override
	public JobSeeker getJobSeekerFromEmail(String emailid) {

		Query query = entityManager.createQuery("SELECT js FROM JobSeeker js WHERE js.emailId = :emailId", JobSeeker.class);
		query.setParameter("emailId", emailid);
		return (JobSeeker) query.getResultList().get(0);
	}
